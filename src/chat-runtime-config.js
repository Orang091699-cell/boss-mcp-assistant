...
export function resolveBossScreeningConfig(workspaceRoot) {
  ...
  return {
    ...
    config: {
      ...
      dimensions: parsed.dimensions && typeof parsed.dimensions === "object" && !Array.isArray(parsed.dimensions)
        ? parsed.dimensions
        : null,
      score_thresholds: parsed.score_thresholds && typeof parsed.score_thresholds === "object" && !Array.isArray(parsed.score_thresholds)
        ? parsed.score_thresholds
        : null,
      recommend: parsed.recommend && typeof parsed.recommend === "object" && !Array.isArray(parsed.recommend)
        ? parsed.recommend
        : null,
      chat: parsed.chat && typeof parsed.chat === "object" && !Array.isArray(parsed.chat)
        ? parsed.chat
        : null
    },
    config_path: configPath,
...
function normalizeLlmThinkingLevel(value) {
  ...
  return ["off", "low", "medium", "high", "current"].includes(normalized) ? normalized : "";
}

export async function setScreeningConfig(configUpdates = {}, workspaceRoot = "") {
  const { config, configPath } = (() => {
    const resolution = getBossScreenConfigResolution(workspaceRoot);
    const targetPath = resolution.writable_path || resolution.resolved_path || resolution.candidate_paths?.[0] || getUserConfigPath();
    const existing = readJsonFile(targetPath) || {};
    return { config: existing, configPath: targetPath };
  })();

  if (configUpdates.baseUrl !== undefined) config.baseUrl = normalizeText(configUpdates.baseUrl);
  if (configUpdates.apiKey !== undefined) config.apiKey = normalizeText(configUpdates.apiKey);
  if (configUpdates.model !== undefined) config.model = normalizeText(configUpdates.model);
  if (configUpdates.llmThinkingLevel !== undefined) {
    const level = normalizeLlmThinkingLevel(configUpdates.llmThinkingLevel);
    if (level) config.llmThinkingLevel = level;
  }
  if (configUpdates.openaiOrganization !== undefined) config.openaiOrganization = normalizeText(configUpdates.openaiOrganization);
  if (configUpdates.openaiProject !== undefined) config.openaiProject = normalizeText(configUpdates.openaiProject);
...
export async function testLlmConnection({ baseUrl, apiKey, model, llmThinkingLevel = "off" } = {}) {
  const normalizedUrl = normalizeText(baseUrl);
  const normalizedKey = normalizeText(apiKey);
  const normalizedModel = normalizeText(model);
  if (!normalizedUrl || !normalizedKey || !normalizedModel) {
    return { ok: false, error: "Missing required LLM config: baseUrl, apiKey, model" };
  }
  ...
  const payload = {
    ...
    messages: [
      { role: "user", content: "Respond with a single word: pass" }
    ],
    max_tokens: 10
  };
  ...
  try {
    const response = await fetch(buildChatCompletionsUrl(normalizedUrl), {
      method: "POST",
      headers: {
    ...
    return {
      ...
      model: json.model || normalizedModel,
    ...
    if (error?.name === "AbortError") {
      return { ok: false, error: "LLM connection timed out after 30s" };
    }
