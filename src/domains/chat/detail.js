...
import {
  ...
  CHAT_RESUME_FILE_ACCEPT_SELECTORS,
...
export async function sendChatRejectionMessage(client, {
  rejectionText = "很抱歉，您过往经历与当前岗位不太匹配，祝您早日找到更合适的机会！",
  dryRun = false
} = {}) {
  const effectiveRejectionText = normalizeDetailText(rejectionText) || "很抱歉，您过往经历与当前岗位不太匹配，祝您早日找到更合适的机会！";
  const initialState = await readChatConversationReadyState(client);
  if (dryRun) {
    return {
      sent: false,
      ...
      reason: "dry_run",
      initial_state: initialState,
      would_send_rejection: true
    };
  }
  const closeBeforeRejection = await closeChatResumeModal(client, { attemptsLimit: 3 });
  if (!closeBeforeRejection.closed) {
    return {
      sent: false,
      skipped: true,
      reason: "resume_modal_close_failed_before_rejection",
      initial_state: initialState,
      close_before_rejection: closeBeforeRejection
    };
  }
  const editorState = await setChatEditorMessage(client, effectiveRejectionText);
  if (!editorState.ok) {
    throw new Error("CHAT_EDITOR_MESSAGE_MISMATCH");
  }
  const sendResult = await sendChatMessage(client, effectiveRejectionText);
  if (!sendResult.sent) {
    throw new Error(`CHAT_REJECTION_SEND_FAILED:${sendResult.error || sendResult.method || "unknown"}`);
  }
  return {
    sent: true,
    skipped: false,
    reason: "rejection_sent",
    initial_state: initialState,
    close_before_rejection: closeBeforeRejection,
    rejection_send_result: sendResult
  };
}

export async function readChatConversationMessages(client) {
  const rootState = await getChatRoots(client);
  let messageNodeId = null;
  let selector = "";
  for (const root of rootState.roots) {
    for (const sel of CHAT_MESSAGE_LIST_SELECTORS) {
      const nodeIds = await querySelectorAll(client, root.nodeId, sel);
      if (nodeIds.length) {
        messageNodeId = nodeIds[0];
        selector = sel;
        break;
      }
    }
    if (messageNodeId) break;
  }
  if (!messageNodeId) return { ok: false, text: "", error: "MESSAGE_LIST_NOT_FOUND" };
  try {
    const html = await getOuterHTML(client, messageNodeId);
    const text = htmlToText(html);
    return { ok: true, text, html_length: html.length, selector };
  } catch (error) {
    return { ok: false, text: "", error: error?.message || "READ_FAILED" };
  }
}

export async function findAndClickChatAcceptResumeButton(client, {
  timeoutMs = 6000,
  intervalMs = 300
} = {}) {
  const started = Date.now();
  ...
  while (Date.now() - started <= timeoutMs) {
    ...
    const target = await findVisibleMatchingTarget(
      ...
      rootState.roots,
      CHAT_RESUME_FILE_ACCEPT_SELECTORS,
      (item) => item.label.includes("接受") || item.label.includes("接收") || item.label.includes("accept")
    );
    lastTarget = target;
    if (target?.node_id && !target.disabled) {
      try {
        if (target.center) {
          await clickPoint(client, target.center.x, target.center.y);
        } else {
        ...
        if (!isRecoverableNodeError(error)) throw error;
        lastTarget = { ...target, click_error: error?.message };
      }
    }
    await sleep(intervalMs);
  }
  return { clicked: false, target: lastTarget };
}

export async function acceptAndReplyAttachmentResume(client, {
  replyText = "收到，稍后我会交给业务评估，如果通过的话会及时和您电话约面",
  dryRun = false
} = {}) {
  const state0 = await readChatConversationReadyState(client);
  if (!state0.has_attachment_resume) {
    return { accepted: false, skipped: true, reason: "no_attachment_resume" };
  }
  const acceptResult = await findAndClickChatAcceptResumeButton(client);
  if (!acceptResult.clicked) {
    return { accepted: false, skipped: true, reason: "accept_button_not_found", accept_attempt: acceptResult };
  }
  if (dryRun) {
    return {
      accepted: false, skipped: false, reason: "dry_run",
      accept_clicked: true, would_send_reply: true
    };
  }
  const effectiveReply = String(replyText || "").trim() || "收到，稍后我会交给业务评估，如果通过的话会及时和您电话约面";
  const closeBeforeReply = await closeChatResumeModal(client, { attemptsLimit: 3 });
  await sleep(300);
  const editorState = await setChatEditorMessage(client, effectiveReply);
  if (!editorState.ok) {
    return { accepted: true, reply_sent: false, reason: "editor_message_mismatch", accept_result: acceptResult };
  }
  const sendResult = await sendChatMessage(client, effectiveReply);
  return {
    accepted: true,
    reply_sent: sendResult.sent,
    reason: sendResult.sent ? "accepted_and_replied" : "reply_send_failed",
...
export async function scrollResumeContent(client, resumeState, {
  maxScrolls = 6,
  wheelDeltaY = 650,
  settleMs = 350
} = {}) {
  const scrollTarget = resumeState?.content
    || resumeState?.popup
    || resumeState?.resumeIframe;
  if (!scrollTarget?.node_id) {
    return { scrolled: false, reason: "no_scrollable_node" };
  }
  await scrollNodeByWheel(client, scrollTarget.node_id, {
    deltaY: Math.max(1, Number(wheelDeltaY) || 1),
    repeat: Math.max(1, Number(maxScrolls) || 1),
    delayMs: Math.max(0, Number(settleMs) || 0)
  });
...
export async function extractChatProfileCandidate(client, {
  ...
  if (!resumeHtml) {
    try {
      ...
      resumeHtml.scroll_before_read = scrollResult;
    } catch (error) {
