...
import { compactScreening, isResumeTooSimple, scoreCandidate, screenCandidate } from "../../core/screening/index.js";
import {
...
export async function runRecommendWorkflow({
  ...
  while (results.length < limit) {
    ...
    if (!screenCandidate?.skip && screeningCandidate?.text?.raw) {
      resumeTooSimpleResult = isResumeTooSimple(screeningCandidate, { minMeaningfulTextLength: 30 });
    }

    await runControl.waitIfPaused();
    runControl.throwIfCanceled();
    runControl.setPhase("recommend:screening");
    const screening = resumeTooSimpleResult?.too_simple
      ? {
          status: "skip",
          passed: false,
          score: 0,
          resume_too_simple: true,
          reasons: ["resume_too_simple"],
          candidate: screeningCandidate
        }
      : screeningConfig?.dimensions
        ? await scoreCandidate({
            candidate: screeningCandidate,
            criteria,
            config: screeningConfig,
            maxRetries: screeningConfig.llmMaxRetries || 3
          })
    ...
    const compactResult = {
      ...
      screening: compactScreening(screening, { defaultDomain: "recommend" }),
  ...
  function startRecommendRun({
    ...
    return manager.startRun({
      ...
      context: {
        ...
        has_screening_config: Boolean(screeningConfig),
        has_dimensions: Boolean(screeningConfig?.dimensions)
      },
      progress: {
        card_count: 0,
