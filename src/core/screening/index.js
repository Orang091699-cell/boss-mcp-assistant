...
export function isResumeTooSimple(candidate, { minMeaningfulTextLength = 30 } = {}) {
  if (!candidate) return { too_simple: true, reason: "no_candidate" };
  const raw = candidate.text?.raw || "";
  const stripped = raw.replace(/【[^】]*】/g, "").trim();
  if (stripped.length >= minMeaningfulTextLength) {
    return { too_simple: false, reason: null, text_length: raw.length, meaningful_length: stripped.length };
  }
  return {
    too_simple: true,
    reason: "resume_too_simple",
    text_length: raw.length,
    meaningful_length: stripped.length,
    min_threshold: minMeaningfulTextLength
  };
}

export function compactScreening(screening, { defaultDomain = "recommend" } = {}) {
  return {
    status: screening.status,
    passed: screening.passed,
    score: screening.score,
    reasons: screening.reasons,
    resume_too_simple: screening.resume_too_simple ?? false,
    dimension_scores: screening.dimension_scores || null,
    recommendation_level: screening.recommendation_level || null,
    evaluation: screening.evaluation || null,
    all_dimensions_passed: screening.all_dimensions_passed ?? null,
    candidate: {
      domain: screening.candidate?.domain || defaultDomain,
...
function buildScoringPrompt(dimensions) {
  const parts = Object.entries(dimensions).map(([name, dim]) => {
    const criteriaList = Array.isArray(dim.criteria) ? dim.criteria.map((c) => `    - ${c}`).join("\n") : "";
    return `  【${name}】(权重 ${dim.weight}, 及格线 ${dim.pass_score})
${criteriaList}`;
  });
  return parts.join("\n");
}
...
export function buildScreeningLlmMessages({
  ...
  let prompt;
  if (mode === "score" && dimensions) {
    const scoringSection = buildScoringPrompt(dimensions);
    prompt =
      `请根据以下维度对候选人进行评分（1-10 分）。\n\n筛选标准:\n${safeCriteria}\n\n`
      + `评分维度:\n${scoringSection}\n\n`
      + `候选人信息:\n${safeText || "候选人的完整简历信息在后续截图中，请按截图顺序阅读。"}\n\n`
      + (images.length
        ? `候选人简历截图共 ${images.length} 张，按从上到下的滚动顺序排列。请完整阅读所有截图后再评分。\n\n`
        : "")
      + "要求：\n"
      + "1) 只能依据候选人信息或截图中真实出现的内容评分。\n"
      + "2) 每个维度评分范围 1-10，1 为完全不匹配，10 为完美匹配。\n"
      + "3) 计算加权总分 total_score = 各维度得分 × 权重之和（四舍五入到一位小数）。\n"
      + "4) 不要输出评估原因、证据列表、解释或额外文字。\n"
      + "5) 只返回 JSON，格式为："
      + "{\"dimension_scores\": {\"技能栈\": 7, \"业务匹配\": 6, ...}, \"total_score\": 6.8, \"evaluation\": \"简要评价\"}";
  } else {
    prompt =
      `请根据以下标准判断候选人是否通过筛选。\n\n筛选标准:\n${safeCriteria}\n\n`
      + `候选人信息:\n${safeText || "候选人的完整简历信息在后续截图中，请按截图顺序阅读。"}\n\n`
  ...
  const userContent = images.length
    ...
    : prompt;
  const systemContent = mode === "score" && dimensions
    ? "你是一位严谨的招聘筛选评分助手。必须完整阅读输入内容，严禁编造不存在的候选人经历。"
      + "按每个维度独立评分，计算加权总分。只能返回严格 JSON，不要输出原因、证据或额外文字。"
...
function parseLlmScoreResponse(parsed, dimensions) {
  if (!parsed || typeof parsed !== "object") return null;
  const dimensionScores = {};
  const dimNames = Object.keys(dimensions || {});
  if (!dimNames.length) return null;
  for (const name of dimNames) {
    const raw = parsed.dimension_scores?.[name];
    const score = Number.isFinite(Number(raw)) ? Number(raw) : NaN;
    if (!Number.isFinite(score) || score < 1 || score > 10) return null;
    dimensionScores[name] = score;
  }
  const totalRaw = Number(parsed.total_score);
  if (!Number.isFinite(totalRaw) || totalRaw < 0 || totalRaw > 10) return null;
  const totalScore = Math.round(totalRaw * 10) / 10;
  const expectedTotal = Math.round(
    dimNames.reduce((sum, name) => sum + dimensionScores[name] * (dimensions[name].weight || 0), 0) * 10
  ) / 10;
  if (Math.abs(totalScore - expectedTotal) > 0.5) return null;
  return {
    dimension_scores: dimensionScores,
    total_score: totalScore,
    evaluation: normalizeText(parsed.evaluation || "")
  };
}
...
export async function callScreeningLlm({
  ...
  try {
    ...
    if (mode === "score" && dimensions) {
      const scored = parseLlmScoreResponse(parsed, dimensions);
      if (!scored) {
        throw new Error(`LLM score response is invalid or missing required fields: ${content.slice(0, 240)}`);
      }
      return {
        ok: true,
        provider: {
        ...
        passed: scored.total_score >= (config.score_thresholds?.pass || 6.0),
...
function computeRecommendationLevel(score, thresholds = {}) {
  const strongRecommend = Number(thresholds.strong_recommend) || 8.5;
  const passScore = Number(thresholds.pass) || 6.0;
  if (score >= strongRecommend) return "S";
  if (score >= passScore) return "A";
  if (score >= passScore * 0.5) return "B";
  return "C";
}

function compactCandidateForScreening(candidate) {
  if (!candidate) return {};
  return {
...
export async function scoreCandidate({
  ...
  const passThreshold = Number(thresholds.pass) || 6.0;
  let lastError = null;
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
    try {
      ...
      const allDimensionPassed = Object.entries(dimensions).every(([name, dim]) => {
        const dimScore = llmResult.dimension_scores?.[name] || 0;
        return dimScore >= (dim.pass_score || passThreshold);
      });
      return {
        schema_version: 1,
        ...
        recommendation_level: computeRecommendationLevel(score, thresholds),
        all_dimensions_passed: allDimensionPassed,
        reasons: [],
        candidate: compactCandidateForScreening(candidate),
  ...
  throw lastError || new Error("scoreCandidate: all retries exhausted");
}
