...
const DEFAULT_INTERN_PERIOD_QUESTION = "你好呀，请问实习周期和最快到岗时间是什么时候呢？";
const DEFAULT_INTERN_RESUME_REQUEST = "同学你好呀，方便发送一份附件简历吗";
const DEFAULT_ATTACHMENT_RESUME_REQUEST = "您好，方便发送一份附件简历吗？";
const DEFAULT_RESUME_ACCEPT_REPLY = "收到，稍后我会交给业务评估，如果通过的话会及时和您电话约面";
...
function buildChatCsvInputRows(snapshot = {}, meta = {}) {
  ...
  return buildLegacyScreenInputRows({
    ...
    extraRows: [
      ["chat_params.greeting_text", normalized.greetingText || meta.args?.greeting_text || meta.args?.greetingText || ""],
      ["chat_params.rejection_text", normalized.rejectionText || meta.args?.rejection_text || meta.args?.rejection || ""],
...
function normalizeRunSnapshot(snapshot) {
  ...
  const oldContext = {
    ...
    greeting_text: meta.normalized?.greetingText || meta.args?.greeting_text || meta.args?.greetingText || "",
...
function normalizeChatStartInput(args = {}) {
  ...
  return {
    ...
    rejectionText: normalizeText(args.rejection_text || args.rejectionText || args.rejection),
    internMode: args.intern_mode === true || args.internMode === true,
    internPeriodQuestionText: normalizeText(args.intern_period_question_text || args.internPeriodQuestionText) || DEFAULT_INTERN_PERIOD_QUESTION,
    internResumeRequestText: normalizeText(args.intern_resume_request_text || args.internResumeRequestText) || DEFAULT_INTERN_RESUME_REQUEST,
    attachmentResumeRequestText: normalizeText(args.attachment_resume_request_text || args.attachmentResumeRequestText) || DEFAULT_ATTACHMENT_RESUME_REQUEST,
    autoAcceptAttachmentResume: args.auto_accept_resume === true || args.autoAcceptAttachmentResume === true,
    resumeAcceptReplyText: normalizeText(args.resume_accept_reply_text || args.resumeAcceptReplyText) || DEFAULT_RESUME_ACCEPT_REPLY,
    target,
...
function getRunOptions(args, normalized, session, { workspaceRoot = "" } = {}) {
  ...
  return {
    ...
    rejectionText: normalized.rejectionText || configFile?.chat?.rejection_text || "",
    ...
    minResumeTextLength: parseNonNegativeInteger(args.min_resume_text_length, 30),
    internMode: normalized.internMode === true,
    internPeriodQuestionText: normalized.internPeriodQuestionText,
    internResumeRequestText: normalized.internResumeRequestText,
    attachmentResumeRequestText: normalized.attachmentResumeRequestText,
    resumeAcceptReplyText: normalized.resumeAcceptReplyText,
    autoAcceptAttachmentResume: normalized.autoAcceptAttachmentResume === true,
