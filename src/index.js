...
function createToolsSchema() {
  return [
    ...
    {
      name: TOOL_SET_SCREENING_CONFIG,
      description: "更新 screening-config.json 中的 baseUrl / apiKey / model，写入后自动验证 LLM 连通性（可通过 skip_test=true 跳过）。只需传入需要修改的字段。",
      inputSchema: {
        type: "object",
        properties: {
          baseUrl: {
            type: "string",
            description: "可选，LLM API 地址，如 https://api.openai.com/v1"
          },
          apiKey: {
            type: "string",
            description: "可选，LLM API Key"
          },
          model: {
            type: "string",
            description: "可选，模型名称，如 gpt-4.1-mini"
          },
          llmThinkingLevel: {
            type: "string",
            enum: ["off", "low", "medium", "high", "current"],
            description: "可选，LLM reasoning 级别"
          },
          openaiOrganization: {
            type: "string",
            description: "可选，OpenAI Organization ID"
          },
          openaiProject: {
            type: "string",
            description: "可选，OpenAI Project ID"
          },
          skipTest: {
            type: "boolean",
            description: "可选，设为 true 跳过配置后自动 LLM 连通性测试"
          }
        },
        additionalProperties: false
      }
...
async function handleSetScreeningConfigTool({ workspaceRoot, args } = {}) {
  const updates = {};
  if (args.baseUrl !== undefined) updates.baseUrl = args.baseUrl;
  if (args.apiKey !== undefined) updates.apiKey = args.apiKey;
  if (args.model !== undefined) updates.model = args.model;
  if (args.llmThinkingLevel !== undefined) updates.llmThinkingLevel = args.llmThinkingLevel;
  if (args.openaiOrganization !== undefined) updates.openaiOrganization = args.openaiOrganization;
  if (args.openaiProject !== undefined) updates.openaiProject = args.openaiProject;
  ...
  if (!Object.keys(updates).length) {
    return {
      ...
      error: {
        code: "NO_UPDATES",
        message: "No config fields provided to update. Specify at least one of: baseUrl, apiKey, model, llmThinkingLevel, openaiOrganization, openaiProject.",
        retryable: true
      }
    };
  }
  ...
  try {
    const writeResult = await setScreeningConfig(updates, workspaceRoot);
    const writtenConfig = writeResult.config;
    const hasRequired = writtenConfig.baseUrl && writtenConfig.apiKey && writtenConfig.model;

    if (args.skipTest || !hasRequired) {
      return {
        status: "OK",
        updated: writeResult.path,
        fields_updated: Object.keys(updates),
        test_skipped: true,
        message: hasRequired
          ? `Config updated at ${writeResult.path}. LLM connectivity test skipped.`
          : `Config updated at ${writeResult.path}. LLM test skipped — missing required fields (baseUrl, apiKey, model).`
      };
    }

    const testResult = await testLlmConnection({
      baseUrl: writtenConfig.baseUrl,
      apiKey: writtenConfig.apiKey,
      model: writtenConfig.model,
      llmThinkingLevel: writtenConfig.llmThinkingLevel || "off"
    });
    ...
    if (testResult.ok) {
      return {
        ...
        test: {
          ...
          finish_reason: testResult.finish_reason,
          usage: testResult.usage
        },
        message: `Config updated and LLM connection verified at ${writeResult.path}.`
      };
    }
    ...
    return {
      ...
      message: `Config written to ${writeResult.path} but LLM connectivity test failed. Check your credentials.`
    };
  } catch (error) {
    return {
      status: "FAILED",
      error: {
        ...
        message: error?.message || "Failed to write screening config",
        retryable: true
      }
    };
  }
...
async function handleRequest(message, workspaceRoot) {
  ...
  if (method === "tools/call") {
    ...
    if (toolName === TOOL_SET_SCREENING_CONFIG) {
      if (args.skipTest !== undefined && typeof args.skipTest !== "boolean") {
        return createJsonRpcError(id, -32602, "skipTest must be a boolean if provided");
      }
      if (args.baseUrl !== undefined && typeof args.baseUrl !== "string") {
        return createJsonRpcError(id, -32602, "baseUrl must be a string if provided");
      }
      if (args.apiKey !== undefined && typeof args.apiKey !== "string") {
        return createJsonRpcError(id, -32602, "apiKey must be a string if provided");
      }
      if (args.model !== undefined && typeof args.model !== "string") {
        return createJsonRpcError(id, -32602, "model must be a string if provided");
      }
    }
    ...
    try {
      ...
      } else if (toolName === TOOL_SET_SCREENING_CONFIG) {
        payload = await handleSetScreeningConfigTool({ workspaceRoot, args });
      } else {
